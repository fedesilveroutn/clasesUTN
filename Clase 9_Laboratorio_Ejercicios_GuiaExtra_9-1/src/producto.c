...
void producto_mostrarMasCaro(sProducto productos[] , int tam)
{
	...
	precioMayor = producto_encotrarPrecioMasCaro(productos, tam);
	...
	if (productos != NULL && tam > 0)
	{
		for (i = 0; i < tam; i++)
		{
			if (productos[i].estado == OCUPADO)
			{
				...
				case IPHONE:
								precioIphone += productos[i].precio;
								contadorIphone++;
								break;

				case MAC:
								precioMac += productos[i].precio;
								contadorMac++;
								break;

				case IPAD:
								precioIpad += productos[i].precio;
								contadorIpad++;
								break;


				case ACCESORIOS:
								precioAccesorios += productos[i].precio;
								contadorAccesorios++;
								break;
				}
			}
			...
			if (contadorIphone != 0)
			{
				resultado = promedio (precioIphone , contadorIphone);
				printf("\nEl precio promedio de los iPhones es de: %.2f", resultado);
			}

			if (contadorMac != 0)
			{
				resultado = promedio (precioMac , contadorMac);
				printf("\nEl precio promedio de los Macs es de: %.2f", resultado);
			}

			if (contadorIpad != 0)
			{
				resultado = promedio (precioIpad , contadorIpad);
				printf("\nEl precio promedio de los Ipads es de: %.2f", resultado);
			}

			if (contadorAccesorios != 0)
			{
				resultado = promedio (precioAccesorios , contadorAccesorios);
				printf("\nEl precio promedio de los Accesorios es de: %.2f", resultado);
			}
